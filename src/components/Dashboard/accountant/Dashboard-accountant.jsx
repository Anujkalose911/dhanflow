...
const Dashboard = () => {
  ...
  const [runTutorial, setRunTutorial] = useState(() => {
    const hasSeenTutorial = localStorage.getItem('hasSeenAccountantDashboardTutorial');
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        try {
          ...
          if (!payload.msme_id) {
            //console.log('No msme_id in token');
        ...
        const response = await axios.get('/get-accountant-dashboard-data', { headers });
  ...
  const handleViewAuditLogs = () => {
    ...
    try {
      ...
      if (!['admin', 'auditor', 'accountant'].includes(payload.role.toLowerCase())) {
  ...
  const kpiData = {
    kpis: [
      ...
      },
      {
        ...
        value: dashboardData.statistics.today_invoices,
        icon: <HiExclamationCircle className="kpi-icon-inner" />,
        type: 'today'
      },
      {
        title: 'Total Approved Amount',
        value: `₹${dashboardData.statistics.total_approved_amount.toLocaleString()}`,
              ...
              dashboardData.recent_invoices.map((invoice, index) => (
                <div key={index} className="activity-item">
                  <div className="invoice-details">
                    ...
                    <span className="amount">₹{invoice.invoice_total_amount.toLocaleString()}</span>
                    <span className={`status ${invoice.approval_status.toLowerCase()}`}>
